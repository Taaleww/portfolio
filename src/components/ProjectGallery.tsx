...
const ProjectGallery: React.FC<Props> = ({ projects }) => {
  return (
    <section id='projects' className='p-10'>
      <div className='py-32 bg-[#F5F5F5] w-full rounded-[24px]'>
        <div className='max-container px-6 md:px-12 lg:px-24'>
          {/* Section Header */}
          <div className='flex flex-col items-center mb-16'>
            <div className='font-script text-[36px] flex items-center justify-center space-x-1.5 text-black mb-1'>
              <span>Scroll to</span>
              <span className='relative inline-block mx-1'>
                <span className='z-11'>explore</span>
                {/* Cyan hand-drawn style circle behind "explore" */}
                <div className='absolute top-1/4 hidden md:block left-[-8%] z-1'>
            ...
            <h2 className='text-5xl md:text-[64px] h-[84px] font-black tracking-[0.05em] uppercase text-black relative'>
              PROJECTS
              {/* Hand-drawn curved arrow */}
              <div className='absolute right-[-50%] top-[-50%] hidden md:block'>
          ...
          <div className='grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-12 lg:gap-y-20 lg:gap-x-20'>
            {projects.map((project) => (
              <a
                ...
                {/* Text Container */}
                <div className='space-y-1 pl-1 flex flex-col'>
                  <div className='flex items-center space-x-2'>
                    <h3 className='text-[24px] font-bold text-black'>{project.title}</h3>
